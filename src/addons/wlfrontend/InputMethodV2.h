...
public:
    InputMethodV2(zwp_input_method_v2 *val, Dim* dim, const std::shared_ptr<wl::client::ZwpVirtualKeyboardV1> &vk);
    ~InputMethodV2() override;

protected:
    void zwp_input_method_v2_activate() override;
    void zwp_input_method_v2_deactivate() override;
    void zwp_input_method_v2_surrounding_text(const char *text, uint32_t cursor, uint32_t anchor) override;
