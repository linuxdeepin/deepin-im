...
#define UTF8_LENGTH(Char)               \
    ((Char) < 0x80                      \
         ...
         : ((Char) < 0x800              \
                ? 2                     \
                : ((Char) < 0x10000 ? 3 \
                                    : ((Char) < 0x200000 ? 4 : ((Char) < 0x4000000 ? 5 : 6)))))
...
static uint32_t fcitx_utf8_get_char_extended(const char *s, int max_len, int *plen)
{
...
size_t fcitx_utf8_strnlen_validated(const char *str, size_t byte)
{
    ...
    while (byte && *str) {
        ...
        uint32_t chr = fcitx_utf8_get_char_validated(
            str,
            (byte > FCITX_UTF8_MAX_LENGTH ? FCITX_UTF8_MAX_LENGTH : byte),
            &charLen);
        if (chr == fcitx::utf8::NOT_ENOUGH_SPACE || chr == fcitx::utf8::INVALID_CHAR) {
...
static int fcitx_ucs4_to_utf8(uint32_t c, char *output)
{
    if (c < 0x00080) {
