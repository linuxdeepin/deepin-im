...
{
    // 创建json对象，读取json文件
    QByteArray allData = readFile(filePath).toLocal8Bit();
...
QString getKeyValue(const int &key)
{
    ...
    QString keyValue_Hex =
        QString("0x%1").arg(keyValue, 2, 16, QLatin1Char('0')); // k为int型或char型都可
