...
{
    return length(std::next(std::begin(s), start), std::next(std::begin(s), end));
}
...
public:
    ...
    ~UTF8CharIterator() = default;
    UTF8CharIterator(const UTF8CharIterator &other) = default;
    UTF8CharIterator &operator=(const UTF8CharIterator &other) = default;
    ...
    std::string_view view() const { return std::string_view{ &*iter_, charLength() }; }
    ...
    bool operator==(const UTF8CharIterator &other) { return iter_ == other.iter_; }

    bool operator!=(const UTF8CharIterator &other) { return !operator==(other); }
