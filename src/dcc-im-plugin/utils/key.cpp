...
std::unordered_map<KeySym, const char *, EnumHash> makeLookupKeyNameMap()
{
    ...
    static const struct
    {
        ...
        { FcitxKey_MonBrightnessDown, NC_("Key name", "Monitor Brightness Down") },
        { FcitxKey_KbdLightOnOff, NC_("Key name", "Keyboard Light On/Off") },
        { FcitxKey_KbdBrightnessUp, NC_("Key name", "Keyboard Brightness Up") },
        { FcitxKey_KbdBrightnessDown, NC_("Key name", "Keyboard Brightness Down") },
...
{
    static const std::unordered_map<KeySym, const char *, EnumHash> map = makeLookupKeyNameMap();
...
#undef _CHECK_MODIFIER
    ...
    QString keyValue = QString::fromStdString(lastModifier);
    if (keyValue.startsWith("<") && keyValue.endsWith(">")) {
        try {
            code_ = std::stoi(keyValue.toStdString().substr(1, keyValue.size() - 2));
...
{
    auto states =
        states_ & KeyStates({ KeyState::Ctrl_Alt_Shift, KeyState::Super, KeyState::Mod3 });
...
{
    return !states_
        && ((sym_ >= FcitxKey_0 && sym_ <= FcitxKey_9)
...
{
    return (sym_ == FcitxKey_Control_L || sym_ == FcitxKey_Control_R || sym_ == FcitxKey_Meta_L
            || sym_ == FcitxKey_Meta_R || sym_ == FcitxKey_Alt_L || sym_ == FcitxKey_Alt_R
            || sym_ == FcitxKey_Super_L || sym_ == FcitxKey_Super_R || sym_ == FcitxKey_Hyper_L
            || sym_ == FcitxKey_Hyper_R || sym_ == FcitxKey_Shift_L || sym_ == FcitxKey_Shift_R);
}

bool Key::isCursorMove() const
{
    return ((sym_ == FcitxKey_Left || sym_ == FcitxKey_Right || sym_ == FcitxKey_Up
             || sym_ == FcitxKey_Down || sym_ == FcitxKey_Page_Up || sym_ == FcitxKey_Page_Down
             || sym_ == FcitxKey_Home || sym_ == FcitxKey_End || sym_ == FcitxKey_KP_Left
             || sym_ == FcitxKey_KP_Right || sym_ == FcitxKey_KP_Up || sym_ == FcitxKey_KP_Down
...
bool Key::isKeyPad() const
{
    return ((sym_ >= FcitxKey_KP_Multiply && sym_ <= FcitxKey_KP_9)
            || (sym_ >= FcitxKey_KP_F1 && sym_ <= FcitxKey_KP_Delete) || sym_ == FcitxKey_KP_Space
            || sym_ == FcitxKey_KP_Tab || sym_ == FcitxKey_KP_Enter || sym_ == FcitxKey_KP_Equal);
}

bool Key::hasModifier() const
{
    return !!(states_ & KeyState::SimpleMask);
}

bool Key::isVirtual() const
{
    return states_.test(KeyState::Virtual);
}
...
Key Key::normalize() const
{
    ...
    key.states_ =
        key.states_
        & KeyStates(
            { KeyState::Ctrl_Alt_Shift, KeyState::Super, KeyState::Mod3, KeyState::Super2 });
    ...
    if (key.states_) {
        ...
        if (Key(key.sym_).isLAZ() || Key(key.sym_).isUAZ()) {
            ...
            if ((key.states_ & KeyState::Shift)
                && (((Key(key.sym_).isSimple() || keySymToUnicode(key.sym_) != 0)
...
#define _APPEND_MODIFIER_STRING(STR, VALUE) \
    ...
    if (format == KeyStringFormat::Portable) {
        ...
        _APPEND_MODIFIER_STRING("Super", (KeyStates{ KeyState::Super, KeyState::Super2 }))
        _APPEND_MODIFIER_STRING("Hyper", (KeyStates{ KeyState::Hyper, KeyState::Hyper2 }))
    } else {
...
{
    const auto *value =
        std::lower_bound(keyValueByNameOffset,
                         keyValueByNameOffset + FCITX_ARRAY_SIZE(keyValueByNameOffset),
                         keyString,
                         [](const uint32_t &idx, const std::string &str) {
    ...
    const auto *compat = std::lower_bound(keyNameListCompat,
                                          keyNameListCompat + FCITX_ARRAY_SIZE(keyNameListCompat),
                                          keyString,
...
std::string Key::keySymToString(KeySym sym, KeyStringFormat format)
{
    ...
    const KeyNameOffsetByValue *result =
        std::lower_bound(keyNameOffsetByValue,
                         keyNameOffsetByValue + FCITX_ARRAY_SIZE(keyNameOffsetByValue),
                         sym,
                         [](const KeyNameOffsetByValue &item, KeySym key) {
                             return item.sym < key;
                         });
    if (result != keyNameOffsetByValue + FCITX_ARRAY_SIZE(keyNameOffsetByValue)
        && result->sym == sym) {
