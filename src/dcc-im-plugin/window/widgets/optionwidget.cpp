...
class IntegerOptionWidget : public OptionWidget
{
    Q_OBJECT
public:
    IntegerOptionWidget(const operation::ConfigOption &option, const QString &path, QWidget *parent)
...
class StringOptionWidget : public OptionWidget
{
    Q_OBJECT
public:
    StringOptionWidget(const operation::ConfigOption &option, const QString &path, QWidget *parent)
        : OptionWidget(path, parent)
    ...
    void writeValueTo(QVariantMap &map) override { writeVariant(map, path(), lineEdit_->text()); }
...
class FontOptionWidget : public OptionWidget
{
    Q_OBJECT
public:
    FontOptionWidget(const operation::ConfigOption &option, const QString &path, QWidget *parent)
        : OptionWidget(path, parent)
        ...
        , defaultValue_(option.defaultValue().variant().toString())
    {
        ...
        connect(fontButton_, &FontButton::fontChanged, this, &OptionWidget::valueChanged);
    ...
    void restoreToDefault() override { fontButton_->setFont(parseFont(defaultValue_)); }
...
class BooleanOptionWidget : public OptionWidget
{
    Q_OBJECT
public:
    BooleanOptionWidget(const operation::ConfigOption &option, const QString &path, QWidget *parent)
        : OptionWidget(path, parent)
        ...
        , defaultValue_(option.defaultValue().variant().toString() == "True")
    {
        ...
        connect(checkBox_, &QCheckBox::clicked, this, &OptionWidget::valueChanged);
...
class KeyListOptionWidget : public OptionWidget
{
    Q_OBJECT
public:
    KeyListOptionWidget(const operation::ConfigOption &option, const QString &path, QWidget *parent)
        : OptionWidget(path, parent)
        , keyListWidget_(new KeyListWidget)
    {
        QVBoxLayout *layout = new QVBoxLayout;
        layout->setMargin(0);
        ...
        keyListWidget_->setAllowModifierLess(
            readString(option.properties(), "ListConstrain/AllowModifierLess") == "True");
        keyListWidget_->setAllowModifierOnly(
            readString(option.properties(), "ListConstrain/AllowModifierOnly") == "True");
        connect(keyListWidget_, &KeyListWidget::keyChanged, this, &OptionWidget::valueChanged);
...
private:
    QList<Key> readValue(const QVariantMap &map, const QString &path)
    {
        int i = 0;
        QList<Key> keys;
        while (true) {
            auto value =
                readString(map, QString("%1%2%3").arg(path).arg(path.isEmpty() ? "" : "/").arg(i));
            if (value.isNull()) {
                break;
            }
...
class KeyOptionWidget : public OptionWidget
{
    Q_OBJECT
public:
    KeyOptionWidget(const operation::ConfigOption &option, const QString &path, QWidget *parent)
        : OptionWidget(path, parent)
        , keyWidget_(new KeySequenceWidget(this))
        , defaultValue_(option.defaultValue().variant().toString().toUtf8().constData())
    {
        QVBoxLayout *layout = new QVBoxLayout;
        layout->setMargin(0);

        keyWidget_->setModifierlessAllowed(readBool(option.properties(), "AllowModifierLess"));
        keyWidget_->setModifierOnlyAllowed(readBool(option.properties(), "AllowModifierOnly"));

        connect(keyWidget_,
                &KeySequenceWidget::keySequenceChanged,
                this,
                &OptionWidget::valueChanged);
    ...
    void restoreToDefault() override { keyWidget_->setKeySequence({ defaultValue_ }); }

private:
    KeySequenceWidget *keyWidget_;
    Key defaultValue_;
};
...
class EnumOptionWidget : public OptionWidget
{
    Q_OBJECT
public:
    EnumOptionWidget(const operation::ConfigOption &option, const QString &path, QWidget *parent)
        : OptionWidget(path, parent)
        ...
        , toolButton_(new QToolButton)
    {
        ...
        while (true) {
            ...
            auto text = readString(option.properties(), QString("EnumI18n/%1").arg(i));
            if (text.isEmpty()) {
                text = value;
            }
            auto subConfigPath =
        ...
        connect(comboBox_, qOverload<int>(&QComboBox::currentIndexChanged), this, [this]() {
    ...
    void readValueFrom(const QVariantMap &map) override
    {
        ...
        toolButton_->setVisible(!comboBox_->currentData(subConfigPathRole).toString().isEmpty());
    }
...
class ColorOptionWidget : public OptionWidget
{
    Q_OBJECT
public:
    ColorOptionWidget(const operation::ConfigOption &option, const QString &path, QWidget *parent)
        : OptionWidget(path, parent)
        , colorButton_(new QPushButton)
    {
        ...
        setLayout(layout);
        connect(colorButton_, &QPushButton::clicked, this, &OptionWidget::valueChanged);

        /* defaultValue_.setFromString(option.defaultValue().variant().toString().toStdString()); */
        defaultValue_ = QColor(option.defaultValue().variant().toString());
    }
    ...
    void readValueFrom(const QVariantMap &map) override
    {
        ...
        colorButton_->setText(qcolor.name());
    }

    void writeValueTo(QVariantMap &map) override
    {
        QColor color /* = colorButton_->pos()*/;
        ...
        writeVariant(map, path(), fcitxColor.name());
    }
...
private:
    ...
    /* fcitx::Color defaultValue_; */
    QColor defaultValue_;
};
...
class ExternalOptionWidget : public OptionWidget
{
    Q_OBJECT
public:
    ExternalOptionWidget(const operation::ConfigOption &option,
                         const QString &path,
                         QWidget *parent)
        : OptionWidget(path, parent)
        ...
        , launchSubConfig_(readBool(option.properties(), "LaunchSubConfig"))
    {
        ...
        connect(button_, &QPushButton::clicked, this, [this, parent, name = option.name()]() {
            if (launchSubConfig_) {
                //                    ConfigWidget *configWidget =
                //                    getConfigWidget(this); if (!configWidget)
                //                    {
                //                        return;
                //                    }
                //                    QPointer<QDialog> dialog =
                //                    ConfigWidget::configDialog(
                //                        this, configWidget->dbus(), uri_,
                //                        name);
                //                    dialog->exec();
                //                    delete dialog;
            } else if (uri_.startsWith("fcitx://config/addon/")) {
                //                    QString wrapperPath =
                //                    FCITX5_QT5_GUI_WRAPPER; if
                //                    (!QFileInfo(wrapperPath).isExecutable()) {
                //                        wrapperPath =
                ...
                //                    }
                //                    args << uri_;
                //                    qCDebug(KCM_FCITX5) << "Launch: " <<
                //                    wrapperPath << args;
                //                    QProcess::startDetached(wrapperPath,
                //                    args);
            } else {
                                    ...
                                    QVariant &result)
{
    ...
    //        configWidget =
    //            new ConfigWidget(parentConfigWidget->description(),
    //            option.type(),
...
QString OptionWidget::prettify(const operation::ConfigOption &option, const QVariant &value)
{
    if (option.type() == "Integer") {
        ...
        /*     if (auto key = option.properties().value("ListDisplayOption").toString(); */
        /*         !key.isEmpty()) { */
        /*         const auto &options = *configWidget->description().find(option.type()); */
