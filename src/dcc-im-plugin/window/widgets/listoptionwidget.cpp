...
public:
    ...
    void readValueFrom(const QVariantMap &map, const QString &path)
    {
        ...
        while (true) {
            auto value =
                readVariant(map, QString("%1%2%3").arg(path).arg(path.isEmpty() ? "" : "/").arg(i));
            if (value.isNull()) {
                break;
            }
            values_ << value;
    ...
    {
        if (!index.isValid() || index.row() >= values_.size() || index.row() == 0) {
            return;
        }
        emit layoutAboutToBeChanged();
        if (!beginMoveRows(index.parent(),
                           index.row(),
                           index.row(),
                           index.parent(),
                           index.row() - 1)) {
...
ListOptionWidget::ListOptionWidget(const operation::ConfigOption &option,
                                   const QString &path,
                                   QWidget *parent)
    : OptionWidget(path, parent)
    , model_(new ListOptionWidgetModel(this))
    , subOption_(option)
{
    setupUi(this);
    ...
    if (props.contains("ListConstrain")) {
        ...
        for (auto iter = itemConstrain.begin(), end = itemConstrain.end(); iter != end; ++iter) {
    ...
    connect(listView->selectionModel(), &QItemSelectionModel::currentRowChanged, this, [this]() {
        updateButton();
    });

    connect(model_, &QAbstractListModel::rowsMoved, this, [this]() {
        updateButton();
    });
    ...
    connect(moveUpButton, &QAbstractButton::clicked, this, [this]() {
        model_->moveUpItem(listView->currentIndex());
    });
    connect(moveDownButton, &QAbstractButton::clicked, this, [this]() {
...
void ListOptionWidget::updateButton()
{
    ...
    moveDownButton->setEnabled(listView->currentIndex().isValid()
                               && listView->currentIndex().row() != model_->rowCount() - 1);
}
