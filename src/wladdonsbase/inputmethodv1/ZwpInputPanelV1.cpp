...
{
    inputPanelSurface_.reset(new ZwpInputPanelSurfaceV1(inputmethodV1_));
    inputPanelSurface_->add(resource->client(), id);
    ...
    if (inputmethodV1_ && inputmethodV1_->panelCreateCallback_) {
        inputmethodV1_->panelCreateCallback_();
    }
}
...
void ZwpInputPanelV1::popupSurfaceCommitNotify(void *data)
{
    ...
    inputmethodV1_->server_->output()->setSize(state.width, state.height);
}
